...
void handle_start_screen_events(SDL_Event* e) {
    if (e->type == SDL_QUIT) {
        ...
        // Start game when '9' key is pressed
...
void render_start_screen() {
    // Clear screen and render start screen background
    ...
    SDL_Rect text_rect = {
        (SCREEN_WIDTH - text_width) / 2,  // Centered horizontally
        SCREEN_HEIGHT * 3/4,  // Positioned in bottom third of screen
        text_width,
        text_height
    };
...
int main(int argc, char* argv[]) {
    ...
    // Open font
    font = TTF_OpenFont("fonts/arial.ttf", 36);  // You'll need to provide a font file
    if (font == NULL) {
